"""
heatmap_module.py - Publication-quality clustered heatmap of significant metabolites.
...
def build_colormap(base_cmap: str = "RdBu_r", custom_colors=None, reverse: bool = False):
    """
    Build a matplotlib colormap either from a predefined name or a custom list of hex colors
    (creates a smooth gradient through them via LinearSegmentedColormap).
    """
    if custom_colors:
...
def clustered_heatmap(data_log2: pd.DataFrame, significant_features, meta: pd.DataFrame = None,
    ...
    row_meta: optional metabolite/feature annotation table (indexed by feature
    ...
    row_annot_cols: list of `row_meta` column names to render as stacked row
          ...
          e.g. a row-annotation file that doesn't cover every metabolite, or a
    ...
    font_family / font_size: base font applied to the title, tick labels, legend
          ...
          proportionally from this one size (e.g. the title is font_size+2, legend
          entries are font_size-3), so changing font_size doesn't require separately
          adjusting each element.
    title_suffix: optional string appended to the title in parentheses, e.g.
          '41 features, FDR ≤ 0.25' -> "Heatmap of Metabolites (41 features, FDR ≤ 0.25)".
          If not given, the title is just "Heatmap of Metabolites".
    ...
    if sub.shape[0] < 2:
        raise ValueError(
            f"Need at least 2 significant metabolites to build a heatmap "
    ...
    if not show_row_labels:
        notes.append(
            f"Row (metabolite) labels hidden — {n_rows} features exceeds the {HIDE_ROW_LABELS_ABOVE} "
    ...
    row_track_label_h_in = 0.34 if has_row_annotation else 0.0  # rotated labels above row-annotation strips
    ...
    H = (title_h_in + row_track_label_h_in + col_dend_h_in + top_gap_in + annot_h_in + annot_gap_in
         + heat_h_in + xtick_label_h_in + cbar_gap_in + cbar_h_in + bottom_margin_in)
