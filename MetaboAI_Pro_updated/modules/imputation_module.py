...
Step 1: assess missingness per metabolite and filter out poor-quality features.
...
"not detected" / below the limit of detection in LC-MS peak area tables).

Implemented methods:
  - Half-Minimum (LOD/2): the most common LC-MS convention — replace with half the
    smallest observed (non-missing) value for that metabolite.
  ...
  - Random Forest (MissForest-style): iteratively predicts each missing value from
    all other metabolites using a random forest regressor
  ...
  - QRILC (approximated): the original QRILC algorithm (quantile regression on
    ...
    LC-MS data are usually left-censored (below the detection limit), not missing at
...
METHOD_INFO = {
    "Half-Minimum (LOD/2)": "Replace missing values with half the minimum observed value for that metabolite. "
                            "The most common convention for LC-MS data.",
    ...
    "QRILC (approximated)": "Left-censored imputation: draws missing values from the lower tail of each "
                            "metabolite's observed distribution, matching the assumption that missing "
                            "LC-MS values are usually below the detection limit rather than random "
...
def compute_missingness(df: pd.DataFrame, treat_zero_as_missing: bool = True) -> pd.DataFrame:
    """
    df: features x samples raw peak area matrix.
...
def impute_qrilc_approx(df: pd.DataFrame, q_min: float = 0.25, shift_factor: float = 1.6,
    ...
    for idx in out.index:
        ...
        draws = truncnorm.rvs(a, b, loc=imputed_mean, scale=imputed_sd, size=n_missing,
                               random_state=rng)
        draws = np.clip(draws, a_min=0, a_max=None)  # peak areas can't be negative
        filled = row.copy()
