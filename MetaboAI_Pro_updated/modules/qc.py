...
def calculate_cv(qc_df: pd.DataFrame) -> pd.DataFrame:
    """
    qc_df: features x QC-samples matrix (raw peak areas).
...
def peak_area_histogram(peak_df: pd.DataFrame, title: str = "Peak Area Distribution"):
    """
    Histogram of all raw peak-area values in a features x samples matrix (every
    feature x every sample cell, flattened), on a log10 scale -- peak areas span
